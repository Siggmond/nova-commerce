...
class AiChatState {
  ...
  ChatSession get activeSession => sessions.firstWhere(
...
class AiChatViewModel extends StateNotifier<AiChatState> {
  ...
  final Ref _ref;
  late final AiChatStore _store = _ref.read(aiChatStoreProvider);
  ...
  Future<void> _hydrate() async {
    ...
    final activeId = sessions.any((s) => s.id == snapshot.activeSessionId)
        ? snapshot.activeSessionId
        : sessions.first.id;

    state = state.copyWith(sessions: sessions, activeSessionId: activeId);
  }

...
AiNavSuggestion _s(AiNavIntent intent, List<double> probs) {
...
void main() {
  ...
  test(
    'mid confidence requires two consecutive identical intents before suggesting',
    () async {
      final runner = _FakeRunner(
        _s(
          AiNavIntent.trends,
      ...
      controller.updateFeatures(
        const [0, 0, 0, 0, 1].map((e) => e.toDouble()).toList(),
      );
      await Future<void>.delayed(const Duration(milliseconds: 1));
  ...
  test(
    'consumeSuggestionAndCooldown clears suggestion and blocks new ones briefly',
    () {
      fakeAsync((async) {
        ...
        final runner = _FakeRunner(
          _s(AiNavIntent.home, [0.82, 0.05, 0.05, 0.03, 0.03, 0.02]),
        );
        final controller = AiNavController(
          modelRunner: runner,
          now: clock,
